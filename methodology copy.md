...
- **Bounded failure.** If the LLM extraction step makes an error, the worst-case outcome is a "needs clarification" or an overly cautious escalation — not an unsafe recommendation acted on directly. The rules engine only acts on fields it received; it does not compensate for or "trust" low-confidence extractions.
