...
3. **Escalates to a human clinician when the extraction layer detects** symptoms suggestive of a complication, or a contradiction/required missing field
...
The pathway logic was informed by prior analysis of anonymized, aggregated clinic-level funnel data. The specific time thresholds used in this prototype (recency windows, path-aware branching for denture vs. single-tooth cases) are conservative working estimates and have not yet been empirically or clinically validated — see `docs/methodology.md` for the full reasoning behind each one.
...
Tested with Python 3.12. No API key needed for the default rule-based extraction mode:
...
**No clinical use is intended.** This is a portfolio/demonstration prototype only — see `docs/safety_and_limitations.md`.

## Evaluation snapshot

- Dataset: 26 hand-written synthetic scenarios
- Data: synthetic only; no real patient records
- Evaluation mode: rule-based fallback extractor (see disclosure below)
- Primary safety check: escalation of symptom-flagged and contradictory cases
- Additional checks: appropriate caution on ambiguous/incomplete cases, consistency, extraction-layer failure modes

Some ambiguous and edge cases returned "insufficient information" rather than the specific expected pathway. This is documented as a limitation of the current extraction method, not treated as a successful classification — see the table below and `docs/methodology.md` for the breakdown.
...
**Evaluation mode: these results were generated using the rule-based fallback extractor. They do not represent a systematic evaluation of the LLM extraction path** (see `docs/methodology.md` section 4.3 for why this distinction matters).
...
Building this prototype reinforced four points:

1. AI is not needed when reliable structured data already exists — a database query handles that case fine.
2. Its actual value here is narrower and more specific: extracting useful facts from unstructured text that structured data doesn't cover.
3. Extraction performance and routing performance need to be evaluated separately — conflating them hides which layer is actually responsible for a given failure.
4. In a safety-sensitive workflow, returning "insufficient information" can be a better outcome than producing a confident guess — and should be measured and credited as such, not treated as a failure to route.
